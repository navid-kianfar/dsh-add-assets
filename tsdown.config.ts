...
const ID = '@achasoft/dsh-add-assets'
...
/**
 * Harness packages the browser half imports as VALUES and the module table cannot answer, so they
 * inline instead. Only pure, runtime-free modules qualify: `@deepseek-ai/dsh-file-reference/grammar`
 * is the `@path` token grammar with no cordis, DOM, or Node dependency of its own. Inlining it is
 * what keeps the mention text this plugin writes identical to the text the harness's own `@` source
 * produces, rather than a second copy of the same rules that can drift from it.
 */
const INLINED_HARNESS_MODULES = new Set(['@deepseek-ai/dsh-file-reference/grammar'])

/**
 * tsc runs first so the emitted `tsbuild/` tree, not the raw sources, is what tsdown bundles. The
 * harness's own build has the same shape (tsc to `lib/types`, then tsdown), which keeps a plugin's
 * emit comparable with the packages it composes against.
 */
 ...
 * @param css - the transformed stylesheet text.
 * @param classMap - exported CSS Modules name to emitted class list.
 * @returns the module source injecting the sheet once and default-exporting the class map.
 */
...
const browserHalf: UserConfig = {
  ...
  // dependencies, and the pure harness modules listed above must inline rather than be required.
  external: (specifier: string) => MODULE_TABLE.has(specifier),
  noExternal: [
    /^(?!react(-dom)?(\/|$)|@deepseek-ai\/).+/,
    ...INLINED_HARNESS_MODULES,
  ],
  define: {
